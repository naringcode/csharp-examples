...
class Program   
{
    class FlagSpinLock
    {
        ...
        public void Acquire() // Enter()
        {
            ...
            {
                // 자원 획득 실패 시 휴식 정책
                // Thread.Sleep(1); // 최소 1ms 대기 후 준비큐에 들어가 스레드를 스케줄링하는 정책
                // Thread.Sleep(0); // 현 스레드보다 우선순위가 높거나 같은 스레드가 있을 경우 양보하는 정책
                // Thread.Yield(); // 대기 중인 스레드가 있으면 양보하는 정책(스레드 우선순위 신경 X)

                // 내부적으로 플랫폼 최적화된 명령어인 PAUSE나 YIELD를 호출한다.

                // 한 번만 스핀하고 내부적으로 플랫폼 최적화된 하이퍼스레딩 관련 작업을 수행한다.
                // 쉽게 생각해서 딱 한 번만 스핀하고, 경우에 따라선 같은 코어에서 동시에 돌아가고 있는 스레드에게 실행을 양보하기 위한 힌트를 주는 정책이다.
                // CPU 내부의 힌트를 적용하기에 위 경우와 같이 무조건적인 양보를 하는 건 아니다.
                Thread.SpinWait(1);

                // 위에 나열된 자획 획득 실패에 따른 정책을 수립하지 않으면 순수하게 스레드의 슬라이스 타임을 모두 소모하는 루프문이 된다.
            }
        }
