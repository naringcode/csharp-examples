...
class Program
{
    static void Main()
    {
        ...
        // C# 8.0 이상부터 지원하는 Pattern Matching 스위치를 통한 할당
        string message = score switch
        {
            100 => "Perfect!",
            int s when score >= 90 => $"A : { s }",
            int s when score >= 80 && score < 90 => $"B : { s }",
            int s when score >= 70 && score < 80 => $"C : { s }",
            int s when score >= 60 && score < 70 => $"D : { s }",
            _ => $"F : { score }"
        };
