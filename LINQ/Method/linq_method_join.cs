...
class Program
{
    ...
    static void Main()
    {
        ...
        // LINQ 메서드 구문은 다른 컬렉션과 키를 기반으로 결합하기 위한 Join()을 지원한다.
        var results = students.Join(
            studentScores, // 결합 대상
            student => student.Id, // 원본 컬렉션의 키
            score => score.StudentId, // 결합 대상이 되는 컬렉션의 키
            (student, score) => (student, score) // 반환 형식(조회 형식)
        );
